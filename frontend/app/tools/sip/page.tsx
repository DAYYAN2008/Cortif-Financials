...
export default function SIPCalculatorPage() {
  ...
  const isValid = !isNaN(monthlyVal) && !isNaN(rateVal) && !isNaN(yearsVal) && monthlyVal > 0 && yearsVal > 0;
    ...
    <div className="max-w-[1400px] mx-auto p-4 lg:p-8 flex flex-col space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 pt-2">
        <div className="lg:col-span-5 flex flex-col space-y-6 rounded-2xl border border-slate-200/80 bg-white/80 p-6 lg:p-8 shadow-sm backdrop-blur-xl dark:border-slate-800/60 dark:bg-slate-900/80">
          <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-2">Investment Parameters</h3>
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Monthly Investment ({glyph})</label>
              <input type="number" value={monthly} onChange={(e) => setMonthly(e.target.value)} placeholder="e.g. 500" className="w-full rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-slate-900 placeholder:text-slate-400 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 dark:border-slate-700/60 dark:bg-slate-950/50 dark:text-white dark:focus:border-purple-400 dark:focus:ring-purple-400/20 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Expected Annual Return (%)</label>
              <input type="number" value={rate} onChange={(e) => setRate(e.target.value)} placeholder="e.g. 12" className="w-full rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-slate-900 placeholder:text-slate-400 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 dark:border-slate-700/60 dark:bg-slate-950/50 dark:text-white dark:focus:border-purple-400 dark:focus:ring-purple-400/20 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Duration (Years)</label>
              <input type="number" value={years} onChange={(e) => setYears(e.target.value)} placeholder="e.g. 10" min="0.1" step="1" className="w-full rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-slate-900 placeholder:text-slate-400 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 dark:border-slate-700/60 dark:bg-slate-950/50 dark:text-white dark:focus:border-purple-400 dark:focus:ring-purple-400/20 transition-all" />
            </div>
          </div>
        </div>
          ...
          {isValid ? (
            <div className="space-y-8 relative z-10">
              <div>
                ...
                <div className="text-5xl lg:text-6xl font-bold tracking-tighter text-slate-900 dark:text-white transition-colors">{formatCurrency(futureValue, currency)}</div>
              </div>
              <div className="flex flex-col sm:flex-row gap-6 pt-4 border-t border-slate-200/60 dark:border-slate-800/60">
                <div className="flex-1">
                  ...
                  <div className="text-2xl lg:text-3xl font-semibold tracking-tight text-slate-700 dark:text-slate-300">{formatCurrency(totalInvested, currency)}</div>
                </div>
                <div className="flex-1">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-2">Wealth Gained</p>
                  <div className="text-2xl lg:text-3xl font-semibold tracking-tight text-emerald-500 dark:text-emerald-400">+{formatCurrency(wealthGained, currency)}</div>
                </div>
              </div>
            </div>
          ) : (
