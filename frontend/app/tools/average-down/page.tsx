...
export default function AverageDownCalculatorPage() {
  ...
  const isValid = !isNaN(currSharesVal) && !isNaN(currPriceVal) && !isNaN(newSharesVal) && !isNaN(newPriceVal) && (currSharesVal > 0 || newSharesVal > 0);
    ...
    <div className="max-w-[1400px] mx-auto p-4 lg:p-8 flex flex-col space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 pt-2">
        <div className="lg:col-span-5 flex flex-col space-y-6 rounded-2xl border border-slate-200/80 bg-white/80 p-6 lg:p-8 shadow-sm backdrop-blur-xl dark:border-slate-800/60 dark:bg-slate-900/80">
          <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-2">Position Details</h3>
          <div className="space-y-6">
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Current Shares</label>
                <input type="number" value={currentShares} onChange={(e) => setCurrentShares(e.target.value)} placeholder="100" className="w-full rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-slate-900 placeholder:text-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-slate-700/60 dark:bg-slate-950/50 dark:text-white dark:focus:border-cyan-400 dark:focus:ring-cyan-400/20 transition-all" />
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Average Price ({glyph})</label>
                <input type="number" value={currentPrice} onChange={(e) => setCurrentPrice(e.target.value)} placeholder="50" className="w-full rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-slate-900 placeholder:text-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-slate-700/60 dark:bg-slate-950/50 dark:text-white dark:focus:border-cyan-400 dark:focus:ring-cyan-400/20 transition-all" />
              </div>
            </div>
            <div className="flex gap-4 pt-2 border-t border-slate-200/50 dark:border-slate-700/50">
              <div className="flex-1 mt-2">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">New Shares to Buy</label>
                <input type="number" value={newShares} onChange={(e) => setNewShares(e.target.value)} placeholder="50" className="w-full rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-slate-900 placeholder:text-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-slate-700/60 dark:bg-slate-950/50 dark:text-white dark:focus:border-cyan-400 dark:focus:ring-cyan-400/20 transition-all" />
              </div>
              <div className="flex-1 mt-2">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">New Purchase Price ({glyph})</label>
                <input type="number" value={newPrice} onChange={(e) => setNewPrice(e.target.value)} placeholder="40" className="w-full rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-slate-900 placeholder:text-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-slate-700/60 dark:bg-slate-950/50 dark:text-white dark:focus:border-cyan-400 dark:focus:ring-cyan-400/20 transition-all" />
              </div>
            </div>
          </div>
        </div>
          ...
          {isValid && totalShares > 0 ? (
            <div className="space-y-10 relative z-10">
              <div>
                ...
                <div className="text-5xl lg:text-6xl font-bold tracking-tighter text-slate-900 dark:text-white transition-colors">{formatCurrency(newAverage, currency)}</div>
              </div>
              <div className="flex flex-col sm:flex-row gap-6 pt-4 border-t border-slate-200/60 dark:border-slate-800/60">
                <div className="flex-1">
                  ...
                  <div className="text-2xl lg:text-3xl font-semibold tracking-tight text-slate-700 dark:text-slate-300">{totalShares.toLocaleString()}</div>
                </div>
                <div className="flex-1">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-2">Total Capital Deployed</p>
                  <div className="text-2xl lg:text-3xl font-semibold tracking-tight text-slate-700 dark:text-slate-300">{formatCurrency(totalCapital, currency)}</div>
                </div>
              </div>
            </div>
          ) : (
